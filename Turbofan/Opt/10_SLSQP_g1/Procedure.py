...
def design_mission(nexus):
    ...
    print('Sweep:  ' + str(vehicle.wings.main_wing.sweeps.quarter_chord))
    print(' ')
    print('----- Results -----')
...
def post_process(nexus):
    ...
    print('MZFW: '+str('%4.1f' % zero_fuel_weight)+' kg, MZFW difference: '+str('%4.1f' % summary.mzfw_consistency))
    ...
    summary.FUEL_HH       = (summary.TOW_HH - max_payload - operating_empty)
    summary.TOW_HH_margin =  summary.FUEL_HH - 6513.

    print('Fuel available - HH: ' + str('%4.1f' % (summary.TOW_HH - max_payload - operating_empty)) + ' kg')
    ...
    summary.design_range_ub = 1340.2 - (results.base.segments['descent_3'].conditions.frames.inertial.position_vector[-1, 0] *
                                       Units.m/Units.nautical_mile)
    summary.design_range_lb = -1340.0 + (results.base.segments['descent_3'].conditions.frames.inertial.position_vector[-1, 0] *
                                       Units.m/Units.nautical_mile)
    ...
    # Objective function
    beta = nexus.beta
    summary.objective = (summary.base_mission_fuelburn / 7060.) * beta + (summary.MTOW / 37200.) * (1 - beta)

    print ('Fuel Burn: ' + str('%4.1f' % summary.base_mission_fuelburn) + ' kg')
    print('MTOW: ' + str('%5.1f' % summary.MTOW))
    print('Beta: '+str('%1.2f' % beta)+' , Objective: ' + str('%1.4f' % summary.objective))
