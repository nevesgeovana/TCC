...
def design_mission(nexus):
    ...
    mission.segments['climb_8'].altitude_end = cruise_altitude
...
def simple_sizing(nexus):
    ...
    HT_volume = 1.15948 * Units.less # E170
    VT_volume = 0.094316 * Units.less # E170
    lht       = 13.8654 * Units.m
    lvt       = 10.5724 * Units.m
    ...
    # landing weight
    landing.mass_properties.landing = 0.97174 * config.mass_properties.takeoff
...
def weight(nexus):
    ...
    vehicle.mass_properties.max_takeoff = 23000. - delta
...
def landing_field_length(nexus):
    ...
    config.mass_properties.landing = nexus.vehicle_configurations.base.mass_properties.max_takeoff * 0.97174
...
def post_process(nexus):
    ...
    print('Fuel Margin: ' + str('%4.1f' % summary.fuel_margin) + ' kg')
    summary.mzfw_consistency      = zero_fuel_weight - operating_empty - max_payload
    summary.BOW                   = operating_empty
    # summary.MTOW = max_takeoff_weight

    # ------------------------------------------------------------------------------------------------------------------
    # Additional Constraints computation
    # TOFL
    summary.takeoff_field_length_margin = 1389.8 - summary.takeoff_field_length
    ...
    # Time to climb
    time_f = results.base.segments['climb_8'].conditions.frames.inertial.time[-1] / Units.min
    time_i = results.base.segments['climb_1'].conditions.frames.inertial.time[0] / Units.min
    summary.time_to_climb = 32.8 - (time_f - time_i)
    ...
    # LFL
    summary.lfl_mlw_margin = 916.65 - summary.landing_field_length
